...
\date{Actualizado al 10-03-2026}
...
Una \textbf{estimación puntual} de un parámetro \(\theta\) es un número único que puede ser considerado como un valor sensible de \(\theta\). Se obtiene una estimación puntual seleccionando un estadístico apropiado y calculando su valor con los datos muestrales dados. El estadístico seleccionado se llama \textbf{estimador puntual} de \(\theta\).
\end{definition}
...
\subsection{Insesgadez}\label{insesgadez}
...
Se dice que un estimador puntual \(\hat{\theta}\) es un \textbf{estimador insesgado} de \(\theta\) si \(E(\hat{\theta}) = \theta\) con todo valor posible de \(\theta\). Si \(\hat{\theta}\) no es insesgado, la diferencia \(E(\hat{\theta}) - \theta\) se conoce como el \textbf{sesgo} de \(\hat{\theta}\).
\end{definition}
...
\begin{example}
Sean \(X_1, X_2, \ldots, X_n\) una muestra aleatoria de una distribución con media \(\mu\) y varianza \(\sigma^2\). Entonces el estimador
...
\begin{definition}[Principio de estimación insesgada con varianza mínima]
Entre todos los estimadores de \(\theta\) insesgados, se selecciona el de varianza mínima. El \(\hat{\theta}\) resultante se llama \textbf{estimador insesgado con varianza mínima (EIVM)} de \(\theta\).
\end{definition}

\begin{exercise}

Si \(X_1, X_2, \ldots, X_n\) representan una muestra aleatoria tomada de una distribución de Rayleigh con función de densidad de probabilidad
...
Si tenemos dos estimadores insesgados, \(\hat{\theta}_1\) y \(\hat{\theta}_2\), la eficiencia relativa de \(\hat{\theta}_1\) respecto a \(\hat{\theta}_2\) se define como:
...
\begin{definition}[Eficiencia basada en ECM]
Si \(\hat{\theta}_1\) y \(\hat{\theta}_2\) son dos estimadores de \(\theta\), la eficiencia relativa basada en el ECM se define como:
\[ ER_{ECM}(\hat{\theta}_1, \hat{\theta}_2) = \frac{ECM(\hat{\theta}_2)}{ECM(\hat{\theta}_1)} \]
Donde \(ECM(\hat{\theta}) = Var(\hat{\theta}) + [E(\hat{\theta}) - \theta]^2\).

Si \(ER_{ECM} > 1\), entonces \(\hat{\theta}_1\) es más eficiente que \(\hat{\theta}_2\).
\end{definition}

\begin{example}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria simple de una población con distribución exponencial \(Exp(\lambda)\), donde \(E[X] = 1/\lambda\) y \(Var(X) = 1/\lambda^2\).
...
Calculamos la eficiencia de \(\widehat{\theta}_1\) respecto a \(\widehat{\theta}_2\):
\[ ER(\hat{\theta}_1, \hat{\theta}_2) = \frac{Var(\hat{\theta}_2)}{Var(\hat{\theta}_1)} = \frac{\theta^2}{\theta^2 / n} = n \]
...
Se dice que un estimador \(\hat{\theta}_n\) es \textbf{consistente} si converge en probabilidad al valor verdadero del parámetro \(\theta\). Matemáticamente, para cualquier \(\epsilon > 0\):
...
\begin{example}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria de una población con media \(\mu\) y varianza \(\sigma^2 < \infty\). El estimador \(\bar{X}_n = \frac{1}{n} \sum X_i\) es consistente para \(\mu\) porque:
\[ E(\bar{X}_n) = \mu \quad \text{y} \quad Var(\bar{X}_n) = \frac{\sigma^2}{n} \]
Al aplicar el límite:
\[ \lim_{n \to \infty} \frac{\sigma^2}{n} = 0 \]
Por la Ley Débil de los Grandes Números, \(\bar{X}_n \xrightarrow{p} \mu\).
\end{example}

\section{El error estándar}\label{el-error-estuxe1ndar}
...
\subsection{Método de momentos (EM)}\label{muxe9todo-de-momentos-em}
...
\begin{definition}
Si \(X_1, \ldots, X_n\) constituyen una muestra aleatoria proveniente de una función masa de probabilidad o de una función de densidad de probabilidad \(f(x)\). Con \(k = 1, 2, 3, \ldots\) el \textbf{k-ésimo momento de la población} o el \textbf{k-ésimo momento de la distribución} \(f(x)\) es \(E(X^k)\). El \textbf{k-ésimo momento muestral} es \(\frac{1}{n} \sum_{i=1}^n X_i^k\).
\end{definition}
...
\begin{example}
Si \(X_1, X_2, \ldots, X_n\) representan una muestra aleatoria de tiempos de servicio de \(n\) clientes en una instalación, donde la distribución subyacente se supone exponencial con el parámetro \(\lambda\). Como sólo hay un parámetro que tiene que ser estimado, el estimador se obtiene igualando \(E(X) = \overline{X}\). Como \(E(X) = 1/\lambda\) con una distribución exponencial, ésta da \(1/\lambda = \overline{X}\) o \(\lambda = 1/\overline{X}\). El estimador de momento de \(\lambda\) es entonces \(\hat{\lambda} = 1/\overline{X}\).
\end{example}

\begin{exercise}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria simple de una población con distribución Bernoulli de parámetro \(p\), donde \(X_i \in \{0, 1\}\). Obtenga el estimador de momentos para \(p\).
\end{exercise}

\begin{exercise}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria de una distribución Uniforme \(U(0, \theta)\), cuya función de densidad es \(f(x) = \frac{1}{\theta}\) para \(0 \leq x \leq \theta\).
\end{exercise}

\begin{exercise}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria de una población con distribución exponencial cuya función de densidad es \(f(x; \theta) = \frac{1}{\theta} e^{-x/\theta}\) para \(x > 0\). Encuentre el estimador de momentos para \(\theta\).
\end{exercise}

\begin{exercise}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria simple de una población con distribución normal \(N(\mu, \sigma^2)\). Obtenga los estimadores de momentos para la media (\(\mu\)) y la varianza (\(\sigma^2\)).
\end{exercise}

\subsection{Método de máxima verosimilitud (EMV)}\label{muxe9todo-de-muxe1xima-verosimilitud-emv}
...
\begin{definition}[Método de máxima verosimilitud]
Que \(X_1, X_2, \ldots, X_n\) tengan una función masa de probabilidad o una función de densidad de probabilidad

\[
f(x_1, x_2, \ldots, x_n; \theta_1, \ldots, \theta_m)
\]

donde los parámetros \(\theta_1, \ldots, \theta_m\) tienen valores desconocidos. Cuando \(x_1, \ldots, x_n\) son los valores muestrales observados y la ecuación anterior se considera como una función de \(\theta_1, \ldots, \theta_m\), se llama \textbf{función de verosimilitud}. Las estimaciones de máxima verosimilitud (emv) \(\hat{\theta}_1, \ldots, \hat{\theta}_m\) son aquellos valores de las \(\theta_i\) que incrementan al máximo la función de verosimilitud, de modo que
...
Cuando se sustituyen las \(X_i\) en lugar de las \(x_i\), se obtienen los \textbf{estimadores de máxima verosimilitud}.
\end{definition}
...
\begin{example}
Suponga que \(X_1, X_2, \ldots, X_n\) es una muestra aleatoria de una distribución exponencial con parámetro \(\lambda\). Debido a la independencia, la función de verosimilitud es un producto de las funciones de densidad de probabilidad individuales:
...
\begin{example}
Sean \(X_1, \ldots, X_n\) una muestra aleatoria de una distribución normal. La función de verosimilitud es
...
Se selecciona una muestra aleatoria de \(n\) cascos para ciclistas fabricados por una compañía. Sea \(X =\) el número entre los \(n\) que están agrietados y sea \(p = P(\text{agrietado})\). Suponga que sólo se observa \(X\), en lugar de la secuencia de \(S\) y \(F\).
...
Sea \(X\) la proporción de tiempo destinado que un estudiante seleccionado al azar pasa resolviendo cierta prueba de aptitud. Suponga que la función de densidad de probabilidad de \(X\) es
...
\begin{exercise}
Sea \(X_1, X_2, \dots, X_n\) una muestra aleatoria simple de una población con distribución de Poisson de parámetro \(\lambda > 0\), cuya función de masa de probabilidad es \(f(x; \lambda) = \frac{e^{-\lambda} \lambda^x}{x!}\) para \(x = 0, 1, 2, \dots\). Encuentre el estimador de máxima verosimilitud para \(\lambda\).
\end{exercise}

\section{Comportamiento del EMV en muestras grandes}\label{comportamiento-del-emv-en-muestras-grandes}
...
Como respuestas a una pregunta de una encuesta a 150 individuos de una muestra se obtuvieron 75 Sí, 55 No y 20 individuos no dieron su opinión.
...
Obtenga el estimador de momentos para cada uno de los siguientes casos:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Distribución Uniforme}: Sea una muestra \(X_i \sim U(\theta_1, \theta_2)\). Obtenga los estimadores para ambos límites del intervalo.
\item
  \textbf{Distribución Geométrica}: \(f(x; p) = p(1-p)^x\) para \(x=0,1,2\dots\). Halle el estimador para la probabilidad de éxito \(p\).
\item
  \textbf{Distribución de Pareto}: \(f(x; \alpha, k) = \frac{\alpha k^\alpha}{x^{\alpha+1}}, x \geq k\). Estime \(\alpha\) asumiendo que \(k\) es conocido (\(E[X] = \frac{\alpha k}{\alpha-1}\) para \(\alpha > 1\)).
\item
  \textbf{Distribución Beta}: \(f(x; \alpha) = \alpha x^{\alpha-1}\) para \(0 < x < 1\). Estime el parámetro de forma \(\alpha\).
\item
  \textbf{Distribución de Potencia}: Sea \(f(x; \theta) = \theta x^{\theta-1}\) para \(0 < x < 1\) y \(\theta > 0\). Obtenga el estimador de momentos para \(\theta\) igualando el primer momento poblacional \(E[X] = \frac{\theta}{\theta+1}\) con la media muestral.
\end{enumerate}
...
Obtenga el estimador de máxima verosimilitud para cada uno de los siguientes casos:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Distribución Binomial}: Sea una muestra de tamaño \(n\) de una Binomial donde el número de ensayos \(m\) es conocido. Estime la probabilidad \(p\).
\item
  \textbf{Distribución Uniforme} \(U(\theta-1, \theta+1)\): Encuentre el estimador de \(\theta\). (Analice el soporte de la función).
\item
  \textbf{Distribución Exponencial desplazada}: \(f(x; \theta) = e^{-(x-\theta)}\) para \(x \geq \theta\). Halle el estimador de \(\theta\).
\item
  \textbf{Distribución de Pareto}: \(f(x; \alpha, k) = \frac{\alpha k^\alpha}{x^{\alpha+1}}, x \geq k\). Obtenga el estimador de \(k\) cuando \(\alpha\) es desconocido (Pista: el soporte depende de \(k\)).
\end{enumerate}

\end{exercise}

\begin{exercise}
Determine el estimador de máxima verosimilitud de la proporción de éxito de una población de 2 características es \(\widehat{p} = X/n\), donde \(X\) es la cantidad de éxitos y \(n\) la cantidad total de observaciones (éxitos + fracasos), para ello, considere la proporción de éxito (probabilidad de éxito) de una distribución Bernoulli.
\end{exercise}

\begin{exercise}
Sea \(X_1, \dots, X_n \sim \text{Poisson}(\lambda)\). Se proponen dos estimadores para \(\lambda\): \(\hat{\lambda}_1 = \bar{X}\) y \(\hat{\lambda}_2 = \frac{X_1 + X_2}{2}\). Demuestre que ambos son insesgados.
\end{exercise}

\begin{exercise}
Sea una muestra \(X_1, \dots, X_n\) de una población con media \(\mu\) y varianza \(\sigma^2\). Demuestre que el estimador de momentos \(S_n^2 = \frac{1}{n} \sum_{i=1}^n (X_i - \bar{X})^2\) es sesgado, calculando que \(E[S_n^2] = \frac{n-1}{n}\sigma^2\).
\end{exercise}

\begin{exercise}
En una distribución Bernoulli(\(p\)), se toma una muestra de tamaño \(n=3\). Se propone el estimador: \(\hat{p} = \frac{1}{4}(X_1 + 2X_2 + X_3)\). ¿Es este un estimador insesgado para \(p\)? Justifique su respuesta mediante el cálculo de la esperanza.
\end{exercise}
...
{\centering \includegraphics{EstadisticaII_files/figure-latex/normalestandar-1} 
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-5}Intervalos de confianza para la media de una distribución normal y varianza poblacional conocida}
\centering
...
{\centering \includegraphics{EstadisticaII_files/figure-latex/simulacionIC-1} 
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/sim-ic1-1} 
...
Utilizando la base de datos del \hyperref[Imacec]{Imacec}:
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-8}Intervalos de confianza para la diferencia de medias de dos distribucones normales y varianzas poblacionales conocidas}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{llc}
\toprule
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-11}Intervalos de confianza para la diferencia de medias de dos distribucones normales y varianzas poblacionales desconocidas e iguales}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{llc}
\toprule
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-12}Intervalos de confianza para la diferencia de medias de dos distribucones normales y varianzas poblacionales desconocidas y distintas}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{llc}
\toprule
...
\begin{exercise}
Utilizando la base de datos del \hyperref[Imacec]{Imacec}, elabore un intervalo de confianza para estudiar si, la media del Imacec del sector de minería es menor al del sector de industria en el periodo 2019-2020. Asuma que, las distribuciones poblacionales son normales e indendientes, y que las varianzas poblacionales son desconocidas y distintas. Utilice una confianza del 96\%. Interprete.
\end{exercise}
...
\begin{exercise}
Utilizando la base de datos del \hyperref[Imacec]{Imacec}, elabore un intervalo de confianza para comparar la variabilidad (varianza) del valor del Imacec entre ambos sectores. Asuma, que las distribuciones poblacionales son normales e independientes. Utilice una confianza del 90\%. Interprete.
\end{exercise}

\begin{exercise}
Utilizando la base de datos del \hyperref[Imacec]{Imacec}, elabore un intervalo de confianza para estudiar la diferencia la media del Imacec de ambos sectores. Asuma, que las distribuciones poblacionales son normales e independientes. Utilice una confianza del 92\%. Interprete.
\end{exercise}

\begin{exercise}
Utilizando la base de datos del \hyperref[Imacec]{Imacec}, elabore un intervalo de confianza para estudiar si, el promedio del Imacec de minería es mayor al de industria por más de 2 unidades. Asuma, que las distribuciones poblacionales son normales e independientes. Utilice una confianza del 97\%. Interprete.
\end{exercise}
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/regiones3-1} 
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/areas-rechazo-ud-1} 
...
{\centering \includegraphics{EstadisticaII_files/figure-latex/region-rechazo-1-1} 
...
\chapter{Comparación de grupos y asociación}\label{comparaciuxf3n-de-grupos-y-asociaciuxf3n}

\section{Análisis de datos categóricos (tablas de contingencia)}\label{anuxe1lisis-de-datos-categuxf3ricos-tablas-de-contingencia}

\subsection{Prueba de bondad y ajuste}\label{prueba-de-bondad-y-ajuste}

\subsection{Prueba chi-cuadrado de homogeneidad}\label{prueba-chi-cuadrado-de-homogeneidad}

\subsection{Prueba chi-cuadrado de independencia}\label{prueba-chi-cuadrado-de-independencia}
...
\section{Métodos no parámetricos}\label{muxe9todos-no-paruxe1metricos}

\subsection{Prueba de Wilcoxon de rangos signados}\label{prueba-de-wilcoxon-de-rangos-signados}

\subsection{Prueba de Mann-Whitney}\label{prueba-de-mann-whitney}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-70-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-71-1} \end{center}
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/regresion1-1} 
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/regresion2-1} 
...
\begin{center}\includegraphics{EstadisticaII_files/figure-latex/unnamed-chunk-74-1} \end{center}
...
\begin{center}\includegraphics{EstadisticaII_files/figure-latex/unnamed-chunk-75-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-78-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-81-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-85-1} \end{center}
...
\begin{example}
\protect\hypertarget{exm:MultipleManual}{}\label{exm:MultipleManual}Utilizando la base de datos \hyperref[Imacec]{Imacec} , se debe considerar un modelo que estudie el valor del Imacec de Minería a base del Imacec de Industria y del Año de medición, con el fin de determinar los beta estimados, los valores ajustados y los errores del modelo, mediante las fórmulas explicadas anteriormente.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =} \FunctionTok{data.frame}\NormalTok{(}\StringTok{"Y\_Gorro"} \OtherTok{=}\NormalTok{ modelo}\SpecialCharTok{$}\NormalTok{fitted.values, }\StringTok{"Residuos"} \OtherTok{=} \FunctionTok{residuals}\NormalTok{(modelo)),}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-103-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/unnamed-chunk-105-1} \end{center}
...
{\centering \includegraphics[width=\linewidth]{EstadisticaII_files/figure-latex/IC-IP-1} 
...
\]
Se entrega la \textbf{matriz de diseño} \(X\) (incluye intercepto), el \textbf{vector de respuestas} \(Y\) y la matriz de parámetros estimados:
\[
X=
\begin{bmatrix}
1 & 1.3 & 2.7\\
...
Obtenga los valores ajustados \(\hat Y=X\hat\beta\). Calcule las SCT, SCReg, SCE, el MCE, el estadístico \(F_0\), \(R^2\), \(R^2\) ajustado y el error estándar residual.
\end{exercise}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Escriba la ecuación de regresión poblacional. Indique la categoría de referencia de las variables cualitativas.
\item
  Escriba la ecuación de regresión ajustada.
\item
...
Se desea ajustar un \textbf{modelo de regresión lineal múltiple con variables cualitativas} para explicar la longitud (cm) del sépalo \(Y=\text{Sepal.Length}\) en función de la anchura (cm) del sépalo \(X_1=\text{Sepal.Width}\) y la especie \(X_2=\text{Species}\) usando la base \textbf{iris} de R. La variable cualitativa \textbf{Species} tiene tres categorías: setosa, versicolor y virginica, con \textbf{setosa} como categoría de referencia. A continuación se muestra el ajuste realizado en R.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Escriba la ecuación de regresión poblacional. Indique la categoría de referencia de la variable cualitativa.
\item
  Escriba la ecuación de regresión ajustada.
\item
  Interprete los parámetros estimados en su contexto y con la codificación usada, indicando unidades y sentido del efecto.
...
A continuación, se muestra una base de datos con las siguientes columnas:
