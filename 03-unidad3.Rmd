...
(\#eq:valor-critico-cada-beta-RLM)
\end{equation}
```
...
A diferencia del estadístico expresado en la ecuación \@ref(eq:estadistico-F-RLS), la expresión del criterio de rechazo para esta prueba es:
...
```{r echo=FALSE}
datos = read.csv("https://raw.githubusercontent.com/Dfranzani/Bases-de-datos-para-cursos/main/2023-1/salariosds.csv")
datos = datos[,c(1,2,7,11)]
colnames(datos) = c("workYear", "experienceLevel", "salaryInUSD", "companySize")
modelo = lm(salaryInUSD ~ workYear + companySize, data = datos)
SCT = round(sum((datos$salaryInUSD - mean(datos$salaryInUSD))^2))
SCReg = round(sum((modelo$fitted.values - mean(datos$salaryInUSD))^2))
```
...
Los valores de SCT y SCReg son $`r SCT`$ y $`r SCReg`$, respectivamente. Suponiendo que se ha utilizado la base de datos mencionada, calcule el R cuadrado ajustado y $F_0$.
:::
...
```{r echo=FALSE}
datos = read.csv("https://raw.githubusercontent.com/Dfranzani/Bases-de-datos-para-cursos/main/2023-1/Heart2.csv")
datos$sex = factor(datos$sex, levels = c("M", "H"))
modelo = lm(oldpeak ~ trtbps + sex, data = datos)
SCT = round(sum((datos$oldpeak - mean(datos$oldpeak))^2))
SCReg = round(sum((modelo$fitted.values - mean(datos$oldpeak))^2))
```

Se ejecutó en R el comando: 

```{r echo=FALSE}
lm(oldpeak ~ trtbps + sex, data = datos)
```

Los valores de SCT y SCReg son $`r SCT`$ y $`r SCReg`$, respectivamente. Suponiendo que se ha utilizado la base de datos mencionada, calcule MCE, MCReg y $F_0$.
:::
...
ggplot(data = df_grafico, aes(x = x, y = y)) + geom_point() +
  ...
  labs(x = "Temperatura" , y = "Ozono", color = "Intervalos",
       title = "Intervalo de confianza (IC) y predicción (IP)")
```
