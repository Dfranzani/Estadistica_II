...
\date{Actualizado al 18-03-2025}
...
\section{Medidas de asociación lineal}\label{medidas-de-asociaciuxf3n-lineal}
...
\item
  ...
  Una característica importante de esta métrica de ajuste es que, tanto el \(R^2\) como el \(\bar{R}^2\) aumentan a medida que se incluyen más covariables en el modelo (consulte el anexo \ref{R2-no-decreciente} para estudiar esta característica). Sin embargo, este último penaliza el valor del \(R^2\) en función de la cantidad de covariables incluidas en este.
\end{itemize}
...
\section{\texorpdfstring{\(R^2\) y \(R^2\) ajustado no decreciente}{R\^{}2 y R\^{}2 ajustado no decreciente}}\label{R2-no-decreciente}
...
Esta conclusión es la misma para el \(R^2\) ajustado (\(\bar{R}^2\)), para ello, observe el siguiente desarrollo a partir de la ecuación \eqref{eq:igualacion-modelos3}.
