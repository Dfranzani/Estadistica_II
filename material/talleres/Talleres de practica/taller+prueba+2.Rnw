...
\begin{enumerate}
  ...
  \item El estimador de mínimos cuadrados de $\beta_0$ es $\widehat{\beta}_0 = \bar{Y} - \widehat{\beta}_1\bar{X}$.
  \item $Y = \bar{Y} + r_{XY}\displaystyle\frac{S_Y}{S_X}(X-\bar{X})$
...
tabla = c(
  ...
  mat_latex("\\displaystyle\\sum_{i=1}^n y\\widehat{y}_i = ", sum.y.por.y.gorro),
