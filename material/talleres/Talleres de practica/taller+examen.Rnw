...
\begin{enumerate}
  ...
  \item Se ejecutó en R el comando: \texttt{lm(charges $\sim$ age + children + sex, data = data)}. Los valores de SCT y SCR son \Sexpr{SCT} y \Sexpr{SCReg}, respectivamente.
  ...
  \item Se ejecutó en R el comando: \texttt{lm(bmi $\sim$ age + smoker, data = data)}. Aquí SCT = \Sexpr{SCT}, SCR = \Sexpr{SCReg}, $S_{\widehat{\beta}_0} =$ \Sexpr{sds[1]}, $S_{\widehat{\beta}_1} =$ \Sexpr{sds[2]} y $S_{\widehat{\beta}_2} =$ \Sexpr{sds[3]}. Además, la ecuación de regresión ajustada es
...
A continuación,haciendo uso de la base de datos explicada en la Parte 2, se despliega una salida de R asociada a un modelo de regresión lineal múltiple.
...
\item Utilice la siguiente tabla (que contiene un resumen de salidas en R) para estudiar los supuestos de Homocedasticidad, Independencia y Normalidad del modelo de la pregunta anterior. Indique la fila del código seleccionado para cada supuesto, justificando su elección, además, escriba las pruebas de hipótesis involucradas e interprete utilizando una confianza del 95\%. 
