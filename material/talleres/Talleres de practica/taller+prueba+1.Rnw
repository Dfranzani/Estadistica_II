...
@
  \begin{enumerate}
    \item Una prueba de hipótesis para determinar si la media del 2014 es menor a 13000 miles de millones de pesos. Utilice un nivel de significancia del 5\%, y considere una desviación estándar poblacional 17210.14. Interprete utilizando el método del valor crítico (asuma que $Z_{0.05, k = 9} = -1.6448$).
